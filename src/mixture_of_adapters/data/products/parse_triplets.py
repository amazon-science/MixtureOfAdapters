...
def parse_triplet(response: str):
    ...
    try:
        ...
        return {
            "anchor_document": triplet["anchorDocument"],
            "id_anchor_document": str(uuid.uuid4()),
            "negative_document": triplet["negativeDocument"],
            "id_negative_document": str(uuid.uuid4()),
            "positive_document": triplet["positiveDocument"],
...
if __name__ == "__main__":
    ...
    s3_inference_paths = [
        f"s3://{bucket_name}/embedding-adapter/data/products/triplets-1745392949/",
    ]

    # Read tasks
    tasks_df = pd.read_parquet(tasks_s3_uri)
    tasks_df = tasks_df.drop_duplicates(subset=["task"])
    print(f"tasks_df columns: {tasks_df.columns.tolist()}")
    ...
    for inf_path in s3_inference_paths:
        ...
        print("Prompt records DataFrame:")
        print(prompt_records.head())

        records = pd.merge(prompt_records, tasks_df, on="task", how="inner")
        ...
        print(f"Triplets columns: {triplets_df.columns.tolist()}")
        print("Triplets DataFrame:")
        print(triplets_df.head())

        records = pd.merge(records, triplets_df, on="recordId", how="inner").drop(columns=["recordId"])
        records_list.append(records)

    # Concatenate all triplet dataframes
    merged_df = pd.concat(records_list, axis=0, ignore_index=True)
