    ...
    if model_args.encoding_type == 'bi_encoder':
        ...
        def preprocess_function(examples):
            if condition_only:
                ...
                input_args = list(map(lambda x: ' '.join([x[0], tokenizer.sep_token, x[1]]),
                                      zip(examples[sentence1_key], examples[sentence2_key])))
            else:
                input_args = list(map(lambda x: ' '.join([x[0], tokenizer.sep_token, x[1], tokenizer.sep_token, x[2]]),
                                      zip(examples[sentence1_key], examples[sentence2_key], examples[condition_key])))
            ...
            if scale is not None:
                ...
                for label in sent1_result['labels']:
                    if (label < _min or label > _max) and label != -1:
                        raise ValueError(f'Label {label} is not in the range [{_min}, {_max}]')
                sent1_result['labels'] = scale_to_range(sent1_result['labels'], _min, _max)
            # Below code only works with v2 dataset
            # if cl_in_batch_neg:
            #     sent4_args = (examples["sim_con_sim_sent_1"],)
            #     sent4_result = tokenizer(*sent4_args, padding=padding, max_length=max_seq_length, truncation=True)
            #     sent5_args = (examples["sim_con_sim_sent_2"],)
            #     sent5_result = tokenizer(*sent5_args, padding=padding, max_length=max_seq_length, truncation=True)
            #     sent1_result['input_ids_4'] = sent4_result['input_ids']
            #     sent1_result['attention_mask_4'] = sent4_result['attention_mask']
            #     sent1_result['input_ids_5'] = sent5_result['input_ids']
            #     sent1_result['attention_mask_5'] = sent5_result['attention_mask']
            #     if 'token_type_ids' in sent4_result:
            #         sent1_result['token_type_ids_4'] = sent4_result['token_type_ids']
            #         sent1_result['token_type_ids_5'] = sent5_result['token_type_ids']
            return sent1_result
    else:
